...
do
  if [ "$echo_only" = "Y" -o "$echo_only" = "y" ]; then
    echo ansible-playbook -i ~/aws-ansible/inventory/$environment/hosts $limit --extra-vars \"region=$region key_name=$key_name\" ~/aws-ansible/playbooks/ping_region_key.yml
  else
    ansible-playbook -i ~/aws-ansible/inventory/$environment/hosts $limit --extra-vars "region=$region key_name=$key_name" ~/aws-ansible/playbooks/ping_region_key.yml 
  fi
